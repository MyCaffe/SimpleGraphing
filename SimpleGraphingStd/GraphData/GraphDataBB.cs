...
namespace SimpleGraphingStd.GraphData
{
    public class GraphDataBB : IGraphPlotData
    {
        ...
        {
            m_config = config;
            m_dfStdDev = config.GetExtraSetting("StdDev", (double)2.0);

            MA ma = (MA)config.GetExtraSetting("MA", (double)0);
        ...
        public BbData Pre(PlotCollection dataset, string strName = null)
        {
            ...
            if (m_config.GetExtraSetting("BbTarget:BarRange", (double)0) == 1)
                m_target = TARGET.BAR;
            else if (m_config.GetExtraSetting("BbTarget:TotalRange", (double)0) == 1)

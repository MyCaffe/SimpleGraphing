...
namespace SimpleGraphingStd
{
    ...
    public class PlotCollection : IEnumerable<Plot>
    {
        ...
        public Plot Add(double dfY, bool bActive = true, int nIdx = 0, bool bSkipMinMax = false)
        {
            lock (m_syncObj)
            {
                Plot p = new Plot(m_dfXPosition, dfY, null, bActive, nIdx);
                m_rgPlot.Add(p);
        ...
        public Plot Add(double dfX, double dfY, bool bActive = true, int nIdx = 0, bool bSkipMinMax = false)
        {
            lock (m_syncObj)
            {
                Plot p = new Plot(dfX, dfY, null, bActive, nIdx);
                m_rgPlot.Add(p);
                Plot last = getLast();
        ...
        public Plot Add(float[] rgfY, bool bActive = true)
        {
            lock (m_syncObj)
            {
                Plot p = new Plot(m_dfXPosition, rgfY, null, bActive);
                m_rgPlot.Add(p);
                m_dfXPosition += m_dfXIncrement;
                Plot last = getLast();
        ...
        public Plot Add(double dfX, float[] rgfY, bool bActive = true)
        {
            lock (m_syncObj)
            {
                Plot p = new Plot(dfX, rgfY, null, bActive);
                m_rgPlot.Add(p);
                Plot last = getLast();
